    ...
    if type == 'main':
        ...
        btns = [
            [InlineKeyboardButton("Go'shtli Shaurma", callback_data=f"{ctg}_goshtli"),
             InlineKeyboardButton("Go'shtli Shaurma achchiq", callback_data=f"{ctg}_goshtlia")],
            [InlineKeyboardButton("Tovuqli Shaurma", callback_data=f"{ctg}_tovuqli"),
             InlineKeyboardButton("Tovuqli Shaurma achchiq", callback_data=f"{ctg}_tovuqlia")],
            [InlineKeyboardButton("🔝Продолжить заказ", callback_data="prodoljit")],
            [InlineKeyboardButton("⬅️ Ortga", callback_data="back")]
        ]
...
def received_inline(update, context):
    ...
    elif data_sp[0] == "waurma":
        if len(data_sp) > 1 and (data_sp[1] == "goshtli" or data_sp[1] == "goshtlia"):
            if len(data_sp) > 2 and data_sp[2] == "klassik":
                print("klassicka keldi")
                ...
                query.message.edit_text("Lavash Hajmini tanlang",
                                        reply_markup=inline_btns("menu_w", ctg=data_sp[0]))
            else:
                query.message.edit_text("Lavash Hajmini tanlang",
                                        reply_markup=inline_btns("lavaw_hajim", l_type=data_sp[1], ctg=data_sp[0]))
        else:
    ...
    elif data_sp[0] == "donar":
        if len(data_sp) > 1 and (data_sp[1] == "goshtli" or data_sp[1] == "tovuqli"):
            if len(data_sp) > 2 and data_sp[2] == "nomer":
                print("combo+ ga keldi")

            elif len(data_sp) > 2 and data_sp[2] == "back":
                query.message.edit_text("Lavash Hajmini tanlang",
                                        reply_markup=inline_btns(type="menu_d", ctg=data_sp[0]))
            else:
                query.message.edit_text("Miqdorini tanlang yoki kiriting",
                                        reply_markup=inline_btns("nomer", l_type=data_sp[1], ctg=data_sp[0]))
        else:
            query.message.edit_text("Miqdorini tanlang yoki kiriting",
                                    reply_markup=inline_btns("menu_d", ctg=data_sp[0]))


    elif data_sp[0] == "burger":
        if len(data_sp) > 1 and (data_sp[1] == "gamburger" or data_sp[1] == "dablburger"):
            if len(data_sp) > 2 and data_sp[2] == "nomer":
                print("combo+ ga keldi")

            elif len(data_sp) > 2 and data_sp[2] == "back":
                query.message.edit_text("Lavash Hajmini tanlang",
                                        reply_markup=inline_btns(type="menu_b", ctg=data_sp[0]))
            else:
                query.message.edit_text("Miqdorini tanlang yoki kiriting",
                                        reply_markup=inline_btns("nomer", l_type=data_sp[1], ctg=data_sp[0]))
        else:
            query.message.edit_text("Miqdorini tanlang yoki kiriting",
                                    reply_markup=inline_btns("menu_b", ctg=data_sp[0]))

    # elif data_sp[0] == "burger":
    #     if len(data_sp) > 1 and (data_sp[1] == "chizburger" or data_sp[1] == "dablchiz"):
    #         if len(data_sp) > 2 and data_sp[2] == "nomer":
    #             print("combo+ ga keldi")
    #
    #         elif len(data_sp) > 2 and data_sp[2] == "back":
